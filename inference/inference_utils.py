...
from config import DEVICE, IOU_THRESHOLD, MAX_OBJECT_CNT, PERSON_CONF, BAG_CONF, XMEM_CONFIG
    ...
    for class_id in range(num_classes):
        while True:
            ...
            for existing_color in color_dict.values():
                distance = ((r - existing_color[0]) ** 2 + (g - existing_color[1]) ** 2 + (b - existing_color[2]) ** 2) ** 0.5
...
def create_mask_from_img(image, person_bboxes, bag_bboxes, save_path=None, sam_checkpoint='./saves/mobile_sam.pt', model_type='vit_t', device=DEVICE):
    ...
    result = masks_on_im([mask.cpu().squeeze().numpy().astype(np.uint8) for mask in all_masks], image)
    result = result[:, :, 0]
...
def get_iou_filtered_yolo_mask_bboxes(boxes1, boxes2, iou_threshold=IOU_THRESHOLD):
    filtered_boxes = []
...
def overlay_mask_on_image(image, mask, class_color_mapping, alpha=0.5):
    ...
    for class_label, color in class_color_mapping.items():
        ...
        result_image[class_mask == 1] = (1 - alpha) * result_image[class_mask == 1] + alpha * np.array(color)

    return result_image

def filter_yolov7_bboxes_by_size(yolov7bboxes, image_size, percentage=10):
    min_bbox_width = image_size[1] * (percentage/100)
    min_bbox_height = image_size[0] * (percentage/100)
